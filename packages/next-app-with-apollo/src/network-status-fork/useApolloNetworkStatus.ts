...
import { ExecutionResult, GraphQLError, OperationTypeNode } from 'graphql'
...
function pendingOperations(type: OperationTypeNode) {
  return function pendingOperationsByType(state: number = 0, action: NetworkStatusAction) {
...
function reducer(state: NetworkStatus, action: NetworkStatusAction): NetworkStatus {
  ...
  // Pending operations
  updatedState.numPendingQueries = pendingQueries(updatedState.numPendingQueries, action)
  updatedState.numPendingMutations = pendingMutations(updatedState.numPendingMutations, action)

  // Latest errors
  updatedState.queryError = queryError(updatedState.queryError, action)
  updatedState.mutationError = mutationError(updatedState.mutationError, action)
...
export default function useApolloNetworkStatus(
  ...
  if (!options) options = {}
  const shouldHandleOperation = options.shouldHandleOperation || defaultShouldHandleOperation

  // Performance optimization to allow changing this option
  // via props without causing the reducer to change.
  const shouldHandleOperationEventCallback = useEventCallback(shouldHandleOperation)
  ...
  return useApolloNetworkStatusReducer(dispatcher, configuredReducer, initialState)
}
